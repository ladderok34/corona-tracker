...
const Countries = () => {
  ...
  const { countries, searchQuery, sortingOption } = useSelector(
    (state) => ({
  ...
  return (
    <Container header={<Header />} centered={searchQuery.length > 0 && !sortedCountries.length}>
      <>
        {searchQuery.length > 0 && !sortedCountries.length ? <NoResults searchQuery={searchQuery} resetSearch={resetSearchDispatch} /> : null}
        {countriesToRender.length > 0 && (
          <CountriesList data={remapCountries(countriesToRender)} listItemOnPress={navigateToCountry} handleEndOnReached={handleEndOnReached} />
        )}
      </>
    </Container>
  )
